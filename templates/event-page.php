...
$manage_mode            = isset( $_GET['agendalite_manage'] ) ? sanitize_text_field( wp_unslash( (string) $_GET['agendalite_manage'] ) ) !== '' : false;
$manage_action          = isset( $_GET['manage_action'] ) ? sanitize_key( wp_unslash( (string) $_GET['manage_action'] ) ) : '';
if ( ! in_array( $manage_action, array( 'reschedule', 'cancel' ), true ) ) {
	$manage_action = '';
}
$manage_booking_id            = isset( $_GET['booking_id'] ) ? (int) sanitize_text_field( wp_unslash( (string) $_GET['booking_id'] ) ) : 0;
$manage_booking_token         = isset( $_GET['booking_token'] ) ? sanitize_text_field( wp_unslash( (string) $_GET['booking_token'] ) ) : '';
$is_return_view               = $payment_return !== '' && $manage_booking_id > 0;
$is_manage_view               = $manage_mode && $manage_action !== '' && $manage_booking_id > 0 && $manage_booking_token !== '';
...
$txt_banner_paypal            = __( 'Procesando tu pago con PayPal.', 'agenda-lite' );
...
$txt_onsite_warning           = __( 'El pago se realizará de forma presencial al momento de la cita.', 'agenda-lite' );
...
$txt_guest_limit_unlimited    = __( 'Sin límite de invitados.', 'agenda-lite' );
...
$txt_manage_loading           = __( 'Cargando datos de tu reserva...', 'agenda-lite' );
$txt_manage_reschedule_title  = __( 'Reagendar reserva', 'agenda-lite' );
$txt_manage_cancel_title      = __( 'Cancelar reserva', 'agenda-lite' );
...
$onsite_note = $price_mode === 'onsite' && (float) $event->price > 0 ? $txt_onsite_warning : '';
?>
<style>
	:root { --lc-accent: <?php echo esc_attr( $accent ); ?>; --lc-accent-text: <?php echo esc_attr( $accent_text ); ?>; }
</style>
<main class="lc-main" role="main">
	<div class="lc-wrap <?php echo $is_return_view ? 'is-return-view' : ''; ?>">
	<div class="lc-card" data-lc-card>
		...
		<?php elseif ( $payment_return === 'paypal' ) : ?>
			<div class="lc-return-banner"><?php echo esc_html( $txt_banner_paypal ); ?></div>
		...
		<?php if ( $is_manage_view ) : ?>
			<div class="lc-manage-box" data-lc-manage-box>
			<div class="lc-step"><?php echo esc_html( $manage_action === 'cancel' ? $txt_manage_cancel_title : $txt_manage_reschedule_title ); ?></div>
			<div class="lc-manage-current" data-lc-manage-current><?php echo esc_html( $txt_manage_loading ); ?></div>
			<div class="lc-manage-policy" data-lc-manage-policy></div>
			<label><?php echo esc_html__( 'Motivo (opcional)', 'agenda-lite' ); ?></label>
			<select class="lc-manage-select" data-lc-manage-reason>
				<option value=""><?php echo esc_html__( 'Seleccionar motivo', 'agenda-lite' ); ?></option>
				<option value="no_puedo"><?php echo esc_html__( 'No puedo asistir', 'agenda-lite' ); ?></option>
				<option value="me_equivoque"><?php echo esc_html__( 'Me equivoqué al reservar', 'agenda-lite' ); ?></option>
				<option value="cambio_planes"><?php echo esc_html__( 'Cambio de planes', 'agenda-lite' ); ?></option>
				<option value="otro"><?php echo esc_html__( 'Otro', 'agenda-lite' ); ?></option>
			</select>
			<?php if ( $manage_action === 'cancel' ) : ?>
				<div class="lc-manage-actions">
				<button class="lc-btn lc-btn-danger" type="button" data-lc-manage-cancel><?php echo esc_html__( 'Confirmar cancelación', 'agenda-lite' ); ?></button>
				</div>
			<?php endif; ?>
			</div>
		<?php endif; ?>
		...
		<?php elseif ( in_array( $price_mode, array( 'partial_percent', 'partial_fixed' ), true ) && (float) $event->price > 0 ) : ?>
			...
			<?php endif; ?>
		<?php elseif ( $price_mode === 'onsite' && (float) $event->price > 0 ) : ?>
			<?php if ( ! empty( $price_sale_display ) ) : ?>
			<div class="lc-payment-summary-row">
				<div class="lc-payment-summary"><s><?php echo esc_html( $price_regular_display ); ?></s></div>
				<div class="lc-payment-summary"><?php echo esc_html( $price_sale_display ); ?></div>
			...
			<div class="lc-payment-summary"><?php echo esc_html( $price_display ); ?></div>
			<?php endif; ?>
			<div class="lc-payment-note-top"><?php echo esc_html( $onsite_note ); ?></div>
		<?php endif; ?>
		<?php if ( $show_description ) : ?>
		<div class="lc-desc"><?php echo wp_kses_post( $event->description ); ?></div>
		<?php endif; ?>
		<?php if ( $has_multiple_employees ) : ?>
			<div class="lc-employee-step" data-lc-employee-step<?php echo ( $is_manage_view && $manage_action === 'reschedule' ) ? ' hidden' : ''; ?>>
...
endforeach;
		...
		<?php
			...
			$guest_limit_legend = $max_guests > 0
				? sprintf(
					/* translators: %d: maximum number of guests allowed for the booking. */
					_n( 'Máximo %d invitado.', 'Máximo %d invitados.', $max_guests, 'agenda-lite' ),
					$max_guests
				)
				: $txt_guest_limit_unlimited;
		?>
		<?php if ( $allow_guests ) : ?>
			...
			<small class="lc-help lc-guests-limit" data-lc-guests-limit><?php echo esc_html( $guest_limit_legend ); ?></small>
			</div>
		<?php endif; ?>
			...
			<?php endif; ?>
			<?php if ( ! empty( $onsite_note ) ) : ?>
			<div class="lc-payment-note"><?php echo esc_html( $onsite_note ); ?></div>
			<?php endif; ?>
	...
	echo wp_json_encode(
		array(
			...
			'manage'                 => array(
				'enabled'       => $is_manage_view ? 1 : 0,
				'action'        => $manage_action,
				'booking_id'    => $manage_booking_id,
				'booking_token' => $manage_booking_token,
			),
		)
	);
	?>
